...
class GCNModelVAE(Model):
    ...
    """
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero,hiddendim,hiddendim2, **kwargs):
...
#Input data loading
mat = pd.read_csv('~path/data/adjmat_data.csv',header=0)
num_nodes=mat.shape[0]
feature_mat = sp.identity(num_nodes)
...
# Add in diagonals
 ...
 #   adj_label = sparse_to_tuple(adj_label)
    ...
    node_distribution = get_distribution('degree', 1.0, adj)  # measure=degree, alpha=1.0(Momentum)
 ...
 # Model training
    ...
    for iter in range(10000):
        ...
        print("Iter:", '%04d' % (iter + 1), "train_loss=", "{:.5f}".format(avg_cost),"train_acc=", "{:.5f}".format(avg_accuracy),
        ...
        valid_roc.append(val_roc)
        valid_ap.append(val_ap)
...
emb = sess.run(model.z_mean, feed_dict = feed_dict)
np.savetxt('~path/data/emb_data.csv', emb, delimiter=',') 


# In[13]:


testedges = pd.read_csv('~path/data/testedgepred_data.csv',index_col=None) # index_col=0
testpred = testedges.to_numpy()
testpred=testpred-1

preds = []
testpred=testpred
for e in range(len(testpred)):
        # Link Prediction on positive pairs()
    preds.append(sigmoid(emb[testpred[e][0],:].dot(emb[testpred[e][1],:].T)))
    
np.savetxt('~path/data/predprob_data.csv', preds, delimiter=',')   
testedges = pd.read_csv('~path/data/testedgepred_data.csv',index_col=None)  
dropoutpos=testedges.iloc[np.where(np.asarray(preds)>0.55)[0]]
...
#Save dropout positions
np.savetxt('~path/data/dropoutpos_data.csv', dropoutpos, delimiter=',') 
